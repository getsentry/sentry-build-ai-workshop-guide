...
- Use **Sentry Seer** to automatically analyze and diagnose errors
- Experience **AI-powered debugging** that can identify root causes across your application
- Leverage **automated fix suggestions** to resolve issues faster than manual debugging
- See how **AI can generate pull requests** with the exact fixes needed to resolve the issue
- Use the **Sentry MCP Server** to bring real-time Sentry context into your development environment
- Access **live issue data** and **Seer analysis** directly from your IDE or AI assistant
...
<Steps>
  1. **Enable MCP Server in Sentry**

     Navigate to your Sentry organization settings and locate the MCP Server integration. Enable it with just two clicks - this will automatically configure the necessary API endpoints and permissions for your development environment.

     <ScaledImage src="/assets/img/sentry-mcp-setup.png" alt="Sentry MCP Setup" size={100} />

  2. **Configure MCP in Your IDE**

     Install the Sentry MCP integration in your IDE (Cursor, VS Code, etc.) to enable direct communication with Sentry's debugging data and Seer analysis.

     ```bash
     # Install the Sentry MCP extension
     cursor --install-extension sentry.sentry-mcp
     ```

  3. **Authenticate with Sentry**

     Use your Sentry authentication token to connect your IDE to your Sentry organization:

     ```bash
     sentry-mcp auth --token YOUR_SENTRY_TOKEN
     ```
...
<Steps>
  1. **Query Sentry Issues via MCP**
     ...
     ```
     Tell me about recent issues related to course search functionality
     ```
