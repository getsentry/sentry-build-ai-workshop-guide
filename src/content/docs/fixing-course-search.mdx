...
2. **Get Seer Analysis for Specific Issue**

   Ask the Sentry MCP server to get the last auto fix data for the specific issue:

   ```
   Use Sentry Seer to analyze the root cause of issue <ISSUE-ID> and get the last auto fix data
   ```
...
3. **Review Seer's Analysis in Cursor**

   You'll see Seer's analysis appear right in your chat window, giving you a clear picture of what's happening.

   You should see:
   - **What's Actually Wrong**: Seer's root cause analysis explaining exactly why the search is failing
   ...
   - **How Sure Seer Is**: Confidence level in the diagnosis
   - **How to Fix It**: Exact code changes needed to resolve the issue

4. **Apply the Fix Locally**

   Once you've reviewed Seer's analysis and you're happy with the suggested solution, just ask Cursor to implement the fix:

   ```
   Apply the Seer fix suggestions to my local codebase
   ```

   Cursor will then:
   - Look through your current code files
   - Apply the exact changes that Seer recommended
...
Seer's automated analysis reveals the issue and provides the exact fix needed. Here's what this powerful debugging experience looks like in action:
...
2. **Cursor Applies the Fix Locally**
   ...
   After reviewing Seer's analysis and you're confident in the solution, ask Cursor to implement the fix:
